...
public class ProfanityFilter {
    ...
    /**
     * Iterates over a String input and checks whether any cuss word was
     * found - and for any/all cuss word found,
     * as long as the cuss word should not be ignored (i.e. check for false
     * positives - e.g. even though "bass"
     * contains the word *ss, bass should not be censored) then (in the
     * String returned) replace the cuss word with asterisks.
     */
    public static String getCensoredText(final String input) {
        ...
        // remove leetspeak
        modifiedInput = modifiedInput.replaceAll("1", "i").replaceAll("!", "i"
        ).replaceAll("3", "e").replaceAll("4", "a")
                .replaceAll("@", "a").replaceAll("5", "s").replaceAll("7", "t"
        ...
        for (int start = 0; start < modifiedInput.length(); start++) {
            ...
            for (int offset = 1; offset < (modifiedInput.length() + 1 - start) && offset < largestWordLength; offset++) {
                String wordToCheck = modifiedInput.substring(start,
                        start + offset);
    ...
    private static void loadBadWords() {
        int readCounter = 0;
        try {
            // The following spreadsheet is from: https://gist.github
            // .com/PimDeWitte/c04cc17bc5fa9d7e3aee6670d4105941
            // (If the spreadsheet ever ceases to exist, then this
            // application will still function normally otherwise - it just
            // won't censor any swear words.)

//            FileReader fr = new FileReader
// ("C:\\Users\\sshah\\Downloads\\Word_Filter - Sheet1.csv");
...
//                            "https://docs.google.com/spreadsheets/d/1hIEi2YG3ydav1E06Bzf2mQbGZ12kh2fe4ISgLg_UBuM/export?format=csv")
            ...
            while ((currentLine = reader.readLine()) != null) {
                ...
                try {
                    ...
                    if (content.length > 1) {
                        ignore_in_combination_with_words = content[1].split(
                                "_");
                    }

                    // Make sure there are no capital letters in the spreadsheet
                    allBadWords.put(word.replaceAll(" ", "").toLowerCase(),
